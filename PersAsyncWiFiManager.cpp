...
void PersWiFiManager::setupWiFiHandlers() {
  ...
  _server->on("/wifi/wps", HTTP_GET, [&](AsyncWebServerRequest *request){
    request->send(200, "text/html", "attempting WPS");
    WiFi.mode(WIFI_STA);
    WiFi.beginWPSConfig();
    delay(100);
    if (WiFi.status() != WL_CONNECTED) {
  ...
  _server->on("/wifi/ap", HTTP_GET, [&](AsyncWebServerRequest *request){
    request->send(200, "text/html", "access point: "+getApSsid());
    startApMode();
  }); //_server->on /wifi/ap

  _server->on("/wifi/rst", HTTP_GET, [&](AsyncWebServerRequest *request){
